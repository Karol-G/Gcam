...
class Tmp():
    ...
    def test_gbp_hook(self):
        model = gcam.inject(self.model, output_dir="results/resnet152/test_gbp_hook", backend="gbp", postprocessor="softmax",
    ...
    def test_gcam_hook(self):
        layer = 'layer4'  # layer4
        model = gcam.inject(self.model, output_dir="results/resnet152/test_gcam_hook", backend="gcam", layer=layer, postprocessor="softmax",
                    evaluate=False, save_scores=False, save_maps=True, save_pickle=True, metric="wioa", dim=2)
        model.eval()
        data_loader = DataLoader(self.dataset, batch_size=1, shuffle=False)
        # TODO: Memory leak finden (Oder nur beim testen?)
    ...
    def test_ggcam_hook(self):
        layer = 'layer4'
        model = gcam.inject(self.model, output_dir="results/resnet152/test_ggcam_hook", backend="ggcam", layer=layer, postprocessor="softmax",
                    evaluate=False, save_scores=False, save_maps=True, save_pickle=True, metric="wioa", dim=2)
        model.eval()
        data_loader = DataLoader(self.dataset, batch_size=1, shuffle=False)
        # TODO: Memory leak finden (Oder nur beim testen?)
    ...
    def test_gcampp_hook(self):
        layer = 'layer4'
        model = gcam.inject(self.model, output_dir="results/resnet152/test_gcampp_hook", backend="gcampp", layer=layer, postprocessor="softmax",
                    evaluate=False, save_scores=False, save_maps=True, save_pickle=True, metric="wioa", dim=2)
        model.eval()
        data_loader = DataLoader(self.dataset, batch_size=1, shuffle=False)
        # TODO: Memory leak finden (Oder nur beim testen?)
