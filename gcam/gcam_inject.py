...
def inject(model, output_dir=None, backend='gcam', layer='auto', postprocessor=None,
    ...
    gcam_dict['Score'] = Score(output_dir + "/", metric=metric, threshold=threshold, layer_ordering=gcam_utils.get_layers(model_clone))
        ...
        for j in range(batch_size):
            for k in range(channels):
                ...
                if self.gcam_dict['evaluate']:
                    ...
                    score = self.gcam_dict['Score'].comp_score(attention_map_single, mask[j][k].squeeze(), layer=layer_name, class_label=k)
